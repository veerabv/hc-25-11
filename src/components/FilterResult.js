...
function FilterResult({ items, selectedAlphabet }) {
  ...
  const [selectedItem, setSelectedItem] = useState(currentItems[0]);

  /* eslint-disable */

  useEffect(() => {
    // if (currentItems.length !== 0) {
    //   setSelectedItem(currentItems[0]); // Select the first item   => filteredItems
    // } else {
  ...
  }, [selectedAlphabet]); //filteredItems
